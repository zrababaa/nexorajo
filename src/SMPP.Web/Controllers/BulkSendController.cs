...
public class BulkSendController : Controller
{
    ...
    {
        await LoadViewBagsAsync(page, ct);
    ...
    public async Task<IActionResult> Submit(BulkSendViewModel model, CancellationToken ct)
    {
        ...
        {
            await LoadViewBagsAsync(1, ct);
            return View(nameof(Index), model);
        }

        try
        {
            var summary = await _bulkSendService.SubmitAsync(_currentUser.UserId, new BulkSendRequest(model.CampaignId, model.Message, model.SenderId), ct);
    ...
    private async Task LoadViewBagsAsync(int page, CancellationToken ct)
    {
